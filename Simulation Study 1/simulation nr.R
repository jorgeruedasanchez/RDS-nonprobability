...
rep=1000
est=matrix(nrow=rep, ncol=9)
colnames(est) <- c("RDS-I", "RDS-II", "RDS-SS", "PSA1", "PSA2", "PSA3", "PSA4", "SM", "DR")
...
for(i in 1:rep){
  ...
  #Estimadores
  est[i,] <- c(est_RDS1[i,], est_RDS2[i,], est_RDSSS[i,], est_PSA1[[1]], est_PSA2[[1]], 
               est_PSA3[[1]], est_PSA4[[1]], est_SM, est_DR)
